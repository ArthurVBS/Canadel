...
<template>
    ...
    <v-card elevation="2" class="d-flex align-center justify-space-between mx-2 card">
      ...
      <v-card-actions class="flex-grow-1 d-flex justify-end pa-2">
        <v-btn
          ...
          @click="hideAddCard"
        />
        <v-btn v-else icon="mdi-plus" title="Shows add product card" @click="showAddCard" />
        <v-btn icon="mdi-sync" title="Fetch all products" @click="fetchAllProducts" />
      </v-card-actions>
    </v-card>
  </v-container>
  ...
  <v-expand-transition>
    <v-container v-show="isAddCardOpen" fluid class="pa-0 mb-4">
      <v-card
        elevation="2"
        class="d-flex flex-column align-center justify-space-between mx-2 py-2 card"
      >
  ...
  <v-container v-else fluid class="pa-1">
    <v-row dense>
      <v-col v-for="product in products" :key="product.id" cols="12" md="6" class="px-2 mb-2">
